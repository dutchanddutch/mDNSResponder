...
/* The source for the hash is derived from OpenSSL files common.h, mem_clr.c, md32_common.h,
 * md5_local.h, md5_dgst.c and md5.h.
 * The following changes have been made to the original sources:
 *    replaced MD5_LONG with mDNSu32
 *    removed conditional #defines and OSSL_DEPRECATEDIN_3_0 macros usage from md5.h
 *    modified memset_t signature and memset_func pointer to use mDNSPlatformMemZero in mem_clr.c
 *    replaced memcpy/memset with mDNSPlatformMemCopy/mDNSPlatformMemZero in md32_common.h
 *
 * Note: machine architecture specific conditionals from the original sources are turned off, but are left in the code
 * to aid in platform-specific optimizations and debugging.
 *
 * Sources originally distributed under the Apache License 2.0.
 *
 */

// from openssl/include/internal/common.h
 ...
 */
typedef void *(*memset_t)(void *, size_t);

static volatile memset_t memset_func = mDNSPlatformMemZero;
...
// from openssl/include/openssl/md5.h
...
// from /openssl/crypto/md5/md5_local.h
...
// from include/crypto/md32_common.h
...
int HASH_UPDATE(HASH_CTX *c, const void *data_, size_t len)
{
    ...
    if (ossl_likely(n != 0)) {
        ...
        if (len >= HASH_CBLOCK || len + n >= HASH_CBLOCK) {
            mDNSPlatformMemCopy(p + n, data, HASH_CBLOCK - n);
             ...
             * security problem.
             */
            mDNSPlatformMemZero(p, HASH_CBLOCK); /* keep it zeroed */
        } else {
            mDNSPlatformMemCopy(p + n, data, len);
    ...
    if (len != 0) {
        p = (unsigned char *)c->data;
        c->num = (unsigned int)len;
        mDNSPlatformMemCopy(p, data, len);
    }
    return 1;
}
...
int HASH_FINAL(unsigned char *md, HASH_CTX *c)
{
    ...
    if (n > (HASH_CBLOCK - 8)) {
        mDNSPlatformMemZero(p + n, HASH_CBLOCK - n);
        n = 0;
        HASH_BLOCK_DATA_ORDER(c, p, 1);
    }
    mDNSPlatformMemZero(p + n, HASH_CBLOCK - 8 - n);
...
// from openssl/crypto/md5/md5_local.h (continued)
...
// from openssl/crypto/md5/md5_dgst.c
...
{
    mDNSPlatformMemZero(c, sizeof(*c));
    c->A = INIT_DATA_A;
    c->B = INIT_DATA_B;
    c->C = INIT_DATA_C;
    c->D = INIT_DATA_D;
